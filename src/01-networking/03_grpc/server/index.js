...
// Next to server/ — works when started from the gRPC example folder (`npm run start:server`).
...
server.bindAsync(
  ...
  (err, port) => {
    if (err) {
      ...
      // run via `npm run start:server` from `03_grpc/`; the server is “up” once the bindAsync callback runs without err and you log the port.
